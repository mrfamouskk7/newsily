const api="5a9ddfd315914f8eb173c24e10764011";
...
function searchFunction() {
  ...
  async function category(val) {
    var url=`https://newsapi.org/v2/top-headlines?category=${val}&apiKey=${api}`;
    if (val.length===2){
      var url=`https://newsapi.org/v2/top-headlines?country=${val}&apiKey=${api}`;
    }
    const response = await fetch(`${url}`);
    const data = await response.json();
    const result = data.articles;
